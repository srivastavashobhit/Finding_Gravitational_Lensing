...
class FeatureDataset(Dataset):
    ...
    def getImg(self, img_path):
        ...
        if '.dcm' in img_path:
            ...
            img2 = cv2.resize(img2, (size_width, size_height))
            ...
            # deal with labels of MR
            if 'MR' in img_path:
                mask2 = cv2.inRange(img2, 110, 200)
                img2 = cv2.bitwise_and(img2, img2, mask = mask2)
                mask = np.stack((mask, mask2))# mask2 = cv2.inRange(img, 240, 255) # spleen's pixel range is 240 - 255
                color_max2 = 110
            

            # img = np.pad(img, ((size_width - img.shape[0]) // 2), self.imgPadding)
            image = img1.reshape(1, img1.shape[0], img1.shape[1])
            image2 = img2.reshape(1, img2.shape[0], img2.shape[1])
            

            # deal with pixels of the label image
            if 1 < image.max():
                image = image // color_max1
            if 1 < image2.max():
                image2 = image2 // color_max2
        ...
        if '.dcm' in img_path:
            ...
            # cv2.imwrite("./imgs/" + "mask" + str(np.sum(mask2)) + '.jpg', mask2.astype(np.uint8))
            # cv2.imwrite("./imgs/" + "img" + str(np.sum(image2)) + '.jpg', 255* image2[0].astype(np.uint8))
            # print("sum1:", np.sum(image))
            # print("sum2:", np.sum(image2))
            return np.concatenate((image, image2))
